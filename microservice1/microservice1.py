...
async def M1(request):
    try:
        async with aiohttp.ClientSession() as session:
            podaci = []
            task = await session.get("http://microservice0:8080/M0")
            #task = await session.get("http://127.0.0.1:8080/M0")
            data = await task.json()

            podaci.append(asyncio.create_task(session.post("http://microservice2:8082/WT1", json=data)))
            podaci.append(asyncio.create_task(session.post("http://microservice3:8083/WT2", json=data)))
